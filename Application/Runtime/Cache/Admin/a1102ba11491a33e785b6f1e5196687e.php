...
<div class="row">
    <div class="col-xs-12">
        ...
        <div class="table-responsive">
            <form action="invitationLists" id="search" method="post">
            ...
            <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                ...
                <?php if(is_array($invitationLists)): foreach($invitationLists as $key=>$row): ?><tr>
                        <td class="center">
                            <label>
                                <input type="checkbox" class="ace" />
                                <span class="lbl"></span>
                        ...
                        <td><?php echo ($row["invitation_code"]); ?></td>
                        <td class="hidden-480"><?php if($row["status"] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
                        <td><?php echo (date('Y-m-d H:m',$row["create_time"])); ?></td>
                        ...
                        <td>
                            <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                ...
                                <a class="red" href="<?php echo U('User/invitationdel');?>?id=<?php echo ($row["id"]); ?>">
...
<script>
    $(function(){
        ...
        $('#sample-table-2_length').append("<select id='records' name='records' onchange='selectpage()' style='float:left'><option <?php if($_COOKIE['records']== 10): ?>selected='selected<?php endif; ?> selected='selected' value='10'>10</option><option <?php if($_COOKIE['records']== 25): ?>selected='selected<?php endif; ?> value='25'>25</option></select>");
        $('#sample-table-search').append("<button id='excelExportFile' style='float:left;'>导出</button><form method='post'action='/Admin/User/uploadExcelFile/' enctype='multipart/form-data' style='float:left'><input type='submit' value='导入' style='float:left;'/><input type='file' name='file_stu' style='float:left;font-size: 16px' /></form>");
...
<script>
    $(function(){
        $("#excelExportFile").click(function(){
            window.open('/Admin/User/excelExport');
        })
    })
