								...
								<div class="row">
									<div class="col-xs-12">
                                        ...
                                        <div class="table-responsive">
                                            <form action="/Admin/Category/lists" id="search" method="post">
											...
											<table id="sample-table-2" class="table table-striped table-bordered table-hover">
                                                ...
                                                <?php if(is_array($categoryData)): $i = 0; $__LIST__ = $categoryData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><input type="hidden" name="id" value="<?php echo ($row["id"]); ?>">
													<tr>
														<td class="center">
															<label>
																<input type="checkbox" class="ace" />
														...
														<td class="hidden-480"><?php if($row["isshownav"] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
														<td class="hidden-480"><?php if($row["iscase"] == 1): ?>案例<?php elseif($row["iscase"] == 0): ?>新闻<?php else: ?>首页导航<?php endif; ?></td>
														...
														<td>
															<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
																...
																<a class="green" href="<?php echo U('Category/categoryUpdate');?>?id=<?php echo ($row["id"]); ?>">
																	<i class="icon-pencil bigger-130"></i>
																</a>

																<a class="red" href="<?php echo U('Category/del');?>?id=<?php echo ($row["id"]); ?>">
                                                                    <input type="hidden" value="<?php echo ($row["id"]); ?>" id="delid">
...
<script>
    function logout(){
        ...
        $.get('http://www.cms2.cn/Admin/Login/logout',data,function(rs){
            if(rs==1){
                alert('退出成功');
                location.href='http://www.cms2.cn/Admin/Login/';
            }else{
                alert('系统错误，请联系管理员');
            }
        });
...
<script>
    ...
    function selectpage(){
        ...
        var data={"records":records};
        $.get('http://www.cms2.cn/Admin/Ajax/articleNumber',data,function(rs,sta){
            if(rs==1){
                location.href='http://www.cms2.cn/Admin/Category/lists';
            }
        })
